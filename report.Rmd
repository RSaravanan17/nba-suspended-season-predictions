...
```{r simulations, include=FALSE, eval=FALSE, warning=FALSE}
...
for (row in 1:nrow(matchups)) {
  ...
  elos <- elos %>%
    mutate(elo = if_else(Team.Name == home, teamA_new_elo,
                         if_else(Team.Name == away, teamB_new_elo, elo)))
...
```{r, results='hide', include=FALSE, warning=FALSE, eval=FALSE, error=FALSE, message=FALSE}
...
```{r final_elo_pred, echo=FALSE, warning=FALSE, eval=FALSE, comment=NA, message=FALSE}
df_melted = melt(elo_history, id.vars='teams')
...
```{r echo=FALSE, out.width='100%', fig.width = 8, fig.height = 5, fig.align='center', fig.cap="\\label{fig:pred_elo}Elo Rating for End of Regular Season and Playoffs", comment=NA, warning=FALSE}
...
Giannis Antetokounmpo led an overwhelming majority of the statistics (WS, VORP, PER, USG%, DBPM). Not only did he lead each of these categories in our end-of-season predictions, he was also in the top three players for the other statistics, including OBPM shown in Table 2.
